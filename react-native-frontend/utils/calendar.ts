import type { CalendarsResponse, EventsRecord, UsersResponse } from "@/api/pocketbase-types";
import type { Colors } from "@/store/Colors";

 type MarkedDates = {
    [date: string]: { periods: { startingDay?: boolean, endingDay?: boolean; color: string}[] }
  }
  
  export  function eventsToMarkedDates(events: (EventsRecord & { color: keyof Colors })[], colors: Colors): MarkedDates {
    ...
    events.sort((a, b) => {
      ...
      const durationA = (new Date(a.endDatetime || a.startDatetime as string).getTime()) - startDateA;
      const durationB = (new Date(b.endDatetime || b.startDatetime as string).getTime()) - startDateB;
      return durationB - durationA;
    });
  
    return events.reduce((markedDates, event) => {
      const { startDatetime, endDatetime = startDatetime, color = "6c60qjalpibtqt9" } = event;
      ...
      for (let currentDate = new Date(startDate); currentDate <= endDate; currentDate.setDate(currentDate.getDate() + 1)) {
        ...
        if (eventIndex === null) {
          eventIndex = markedDates[dateStr].periods.push({
  ...
  export const updateCalendarState = (
    prev: CalendarsResponse<{ users: UsersResponse[] }>[],
    updatedCalendar: CalendarsResponse<{ users: UsersResponse[] }>,
  ) =>
    [updatedCalendar, ...prev.filter((c) => c.id !== updatedCalendar.id)].sort(
